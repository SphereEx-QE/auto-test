...
public final class ShardingJdbcStatementExecuteUpdateTest extends ShardingJdbcBaseTest {
    ...
    public ShardingJdbcStatementExecuteUpdateTest() throws Exception {
    ...
    public boolean run() throws SQLException {
        boolean r1 = executeUpdateWithOnlySql();
        boolean r2 = executeUpdateWithSQLAndAutoGeneratedKeys();
        boolean r3 = executeUpdateWithSQLAndColumnNames();
        if (!r1 || !r2 || !r3) {
            return false;
        }
        return true;
    ...
    private boolean executeUpdateWithOnlySql() throws SQLException {
    ...
    private boolean executeUpdateWithSQLAndAutoGeneratedKeys() throws SQLException {
        ShardingSphereConnection conn = (ShardingSphereConnection) getAutoDataSource().getConnection();
        conn.setAutoCommit(false);
        if (conn.getConnectionManager().getConnectionTransaction().isRollbackOnly()) {
            logger.error("expect transaction is not rollback only, but transaction rollback only.");
    ...
    private boolean executeUpdateWithSQLAndColumnNames() throws SQLException {
        ShardingSphereConnection conn = (ShardingSphereConnection) getAutoDataSource().getConnection();
        conn.setAutoCommit(false);
        if (conn.getConnectionManager().getConnectionTransaction().isRollbackOnly()) {
            logger.error("expect transaction is not rollback only, but transaction rollback only.");
